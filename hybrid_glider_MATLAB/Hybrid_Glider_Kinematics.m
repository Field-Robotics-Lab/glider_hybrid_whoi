...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%     20-07-26 Woensug Choi    %%%%%%
%%%%    Mixture of Brian and Isa  %%%%%%
%%%%  Uses Brian's exp input data %%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% --- Changelog --- %
% 20-08-03 Notation matched with overleaf 
...
%plot(time,eta(2,:),'r')
subplot(2,1,1)
plot(m_present_time,-m_pitch.*180./(2*pi()),' *g')
hold on
plot(time,eta(5,:).*180./(2*pi()),'k')
hold on
...
plot(time,eta(1,:),'r')
hold on
plot(m_present_time,m_depth,' *c')
plot(time,-eta(3,:),'k')
plot(m_present_time,m_depth_rate.*100,' *b')
plot(time,-eta_dot(3,:).*100,'g')
% plot(time,x_s_c.*1000,'c')
% plot(time,V_B.*100000,'b')
ylim([-100 100])
legend('x','z_{glider}','z_{sim}','z_{dot glider}','z_{dot sim}')