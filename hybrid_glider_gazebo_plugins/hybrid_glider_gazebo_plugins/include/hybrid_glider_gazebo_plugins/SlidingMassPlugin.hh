...
/// \file SlidingMassPlugin.hh
/// \brief Model plugin for description of a glider's sliding mass.

#ifndef __HYBRID_GLIDER_GAZEBO_PLUGINS_SLIDINGMASS_PLUGIN_HH__
#define __HYBRID_GLIDER_GAZEBO_PLUGINS_SLIDINGMASS_PLUGIN_HH__
...
class SlidingMassPlugin : public ModelPlugin
{
    /// \brief Constructor
    public: SlidingMassPlugin();

    /// \brief Destructor
    public: virtual ~SlidingMassPlugin();
    ...
    /// \brief SlidingMass dynamic model
    ...
    protected: transport::PublisherPtr massPosPublisher;

    /// \brief Latest input command.
    protected: double inputCommand;

    /// \brief SlidingMass ID
    protected: int slidingmassID;
    ...
    protected: common::Time massPosStamp;
};
}
