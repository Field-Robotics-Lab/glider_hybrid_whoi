...
#include <hybrid_glider_gazebo_plugins/HydrodynamicModel.hh>
  ...
  double _time, const ignition::math::Vector3d &_flowVelWorld)
{
  ...
  if ( this->debugFlag )
  {
    ...
    this->StoreVector(HYBRID_GLIDER_DAMPING_FORCE, Vec3dToGazebo(damping.head<3>()));
    this->StoreVector(HYBRID_GLIDER_DAMPING_TORQUE, Vec3dToGazebo(damping.tail<3>()));

    this->StoreVector(HYBRID_GLIDER_ADDED_MASS_FORCE, Vec3dToGazebo(added.head<3>()));
    this->StoreVector(HYBRID_GLIDER_ADDED_MASS_TORQUE, Vec3dToGazebo(added.tail<3>()));

    this->StoreVector(HYBRID_GLIDER_ADDED_CORIOLIS_FORCE, Vec3dToGazebo(cor.head<3>()));
    this->StoreVector(HYBRID_GLIDER_ADDED_CORIOLIS_TORQUE, Vec3dToGazebo(cor.tail<3>()));
  }
}
