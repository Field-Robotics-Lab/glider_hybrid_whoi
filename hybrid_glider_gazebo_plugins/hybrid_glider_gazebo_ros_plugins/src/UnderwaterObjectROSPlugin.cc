...
#include <hybrid_glider_gazebo_ros_plugins/UnderwaterObjectROSPlugin.hh>
  ...
  hybrid_glider_gazebo_ros_plugins_msgs::GetModelProperties::Response& _res)
{
  for (std::map<gazebo::physics::LinkPtr,
       hybridglider::HydrodynamicModelPtr>::iterator it = models.begin();
       it != models.end(); ++it)
  {
    gazebo::physics::LinkPtr link = it->first;
    hybridglider::HydrodynamicModelPtr hydro = it->second;

    _res.link_names.push_back(link->GetName());

    hybrid_glider_gazebo_ros_plugins_msgs::UnderwaterObjectModel model;
    double param;
