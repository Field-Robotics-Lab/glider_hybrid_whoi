...
def command(startTime):
    while not rospy.is_shutdown():
        if rospy.get_time()-startTime >= 1:  # initiate after 2 seconds
            ...
            command.target_pitch_value = -0
            command.target_pumped_volume = 0
            command.rudder_control_mode = 1
            command.target_heading = math.pi/2
            command.motor_cmd_type = 1
            command.target_motor_cmd = 0.5
...
if __name__ == '__main__':
    try:
        # start node
        pub = rospy.Publisher('/glider_asv/direct_kinematics/UwGliderCommand', UwGliderCommand, queue_size=10)
